...
TEST(CappedWeightedSuffixTree, DIFFERENT_MIN_MAX_VALUES_IN_LEAF) {
    ...
    int isa[dsSize] = {};
    int lcp[dsSize] = {};
    lz77::LZ77CompactTries<uint>::DS::constructSuffixArray(buffer.c_str(), sa, dsSize);
    lz77::LZ77CompactTries<uint>::DS::constructInverseSuffixArray(sa, isa, dsSize);
    lz77::LZ77CompactTries<uint>::DS::constructLcpArray(buffer.c_str(), sa, isa, lcp, dsSize);
    int *pSa = &sa[0];
    int *pLcp = &lcp[0];
