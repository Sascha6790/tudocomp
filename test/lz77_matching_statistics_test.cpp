...
TEST(LZ77SuffixSorting, SuffixSortingInitialTest) {
    ...
    auto result = test::compress<lz77::LZ77MatchingStatistics<lzss::DidacticalCoder>>(input, "HASH_BITS=4");
    ASSERT_EQ(result.str, "cbabc{3, 11}cbabc{3, 3}{8, 8}abc{3, 6}b{10, 6}");
}

TEST(LZ77MatchingStatistics, test) {
    ...
    ;
    auto result = test::compress<lz77::LZ77MatchingStatistics<lz77::LZ77StreamingCoder<ASCIICoder, ASCIICoder, ASCIICoder>>>(input, "HASH_BITS=15");
    decompress<lz77::LZ77MatchingStatistics<lz77::LZ77StreamingCoder<ASCIICoder, ASCIICoder, ASCIICoder>>>(result.str, input, "");
}